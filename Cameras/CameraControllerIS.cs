...
namespace OneKnight.Cameras {
    public class CameraControllerIS : MonoBehaviour {
        ...
        public void OnPan(InputValue input) {
            if(!enableInputs)
                return;
            Vector2 panAmount = input.Get<Vector2>();
            //switch(mode) {
            //    case Mode.Focus:
            //        break;
            //    case Mode.Free:
            //        if(panAllowed) {
            //            PanView(-panSpeedFactor*(transform.forward*Utils.PowInterp(panAmount.y, 1) + transform.right*Utils.PowInterp(panAmount.x, 1)));
            //        }
            //        break;
            //}
            if(panAllowed) {
                CancelSelect();
                MoveView(panAmount.x, panAmount.y);
            }
        }

        public void OnRotate(InputValue input) {
            if(!enableInputs || !rotationAllowed)
                return;
            Vector2 rotateAmount = input.Get<Vector2>();
            if(followInPlane != null || rotateWithoutFocus)
                RotateCamera(rotationSpeed*rotateAmount.x, rotationSpeed*rotateAmount.y);
        }

        public void OnZoom(InputValue input) {
            if(!enableInputs)
                return;
            Vector2 zoomAmount = input.Get<Vector2>();
            switch(mode) {
                case Mode.Focus:
                    if(zoomAllowed) {
                        if(cam.orthographic)
                            ZoomOrtho(zoomAmount.y);
                        else
                            Zoom(zoomAmount.y);
                    }
                    break;
        ...
        public void OnChangeCameraSpeed(float input) {
            float speedChange = input;
            speedExponent += speedExponentChangeSpeed*speedChange;
        ...
        void Update() {
            if(autoRotate && (followInPlane != null || autoWhileUnfocused)) {
                RotateCamera(Time.unscaledDeltaTime*autoRotateSpeed, 0);
            }
        }
        ...
        IEnumerator SmoothCameraToTarget(Transform target, float seconds) {
            ...
            for(float t = 0; t < seconds && currentSelected != null; t += Time.unscaledDeltaTime) {
